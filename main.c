...
int main()
{
	...
	Window window;
	window_init(&window, 600, 600, "Pendulum");

	int32_t v_length = (window.width / 2 < window.height) ? window.width / 2 : window.height;
	...
	while (playing) {
		window_clear(&window);
		for (int32_t i = 0; i < NUM_PENDULUM; i++) {
			int32_t x = pendulum_get_x(&pends[i]) * v_length + window.width / 2;
			int32_t y = pendulum_get_y(&pends[i]) * v_length;
			window_draw_line(&window, window.width / 2, 0, x, y, 255, 255, 255);
